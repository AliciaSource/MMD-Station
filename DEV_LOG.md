...
## 2026-08-23 - V0.1.8 物理预览隔离显示、Native 批处理与 60 FPS 量级候选

- 从用户已验收的提交 `292dcb3` / `baseline-20260823-mmd-preview-pose-pipeline` 继续，目标限定为 `CURRENT_PROXY` Pose Mode 操作性能，同时守住 type 0 同 tick、动态链、Root/Bone、MMD IK、Undo/Redo 与 Reset 语义。剖析确认主要瓶颈仍在 Blender host adapter 的完整 Armature/Modifier 求值与 250 Rigid + 374 Joint 的逐 Object 调试显示，不是 Rust/Bullet 固定步本身。
- 新增 `physics_preview/display_rig.py`：符合条件的单 Session 在原 Scene 内创建隐藏的精简 Armature，只保留受物理影响的骨骼；显示 Object 与源 Object 共用原 Mesh datablock、保留 Armature modifier 配置但把目标重定向到精简 Armature，源 Object 暂时隐藏。canonical Armature 因而只保留用户控制输入，物理结果不再写回全部 868 根骨骼，也不复制 Mesh 顶点或 custom normals。linked data、多 Scene 共享、B-Bone、额外约束、隐藏源对象或不兼容 Modifier 栈均 fail closed 回退旧 canonical 路径。
- 新增 `physics_preview/debug_batch.py`：把原 Rigid/Joint 暂存到 owner-scoped 隐藏 Collection，按 kinematic/slow/static 域合并成 5 个 helper Mesh；0 型 Rigid 仍每个有效显示 tick 同步，动态对象按调试 cadence 更新，静态对象只在结构事件重建。Joint `ARROWS` 使用每对象 13 vertices / 12 edges 的 +X/+Y/+Z 定向轮廓并保持 `empty_display_size`；其他 Empty 显示类型在修改 Scene 前直接回退 per-object。batch 仅在 Pose Mode 启用；退出 Pose Mode 会先强制本 tick 调试输出并把最终矩阵写回源对象，再恢复 Collection、`hide_viewport` 与 `hide_select`，Object Mode 逐对象选择和位置语义均不丢失。
- MMD IK 桥接新增直接 Native PoseProvider，兼容时从 native 层级姿态直接提供 physics 输入，避免把整套结果先写回 canonical Armature 再读出。`mmd_bone_solver.dll` 新增 bone transform、rigid target、live matrix 与 external rigid matrix 的批量入口，Python 侧复用严格校验的 flat buffers；缺少新符号时逐级回退既有 scalar API。
- PMX/MMD 两个物理 DLL 都新增可选 `mmd_solver_set_bone_targets_pmx_eulers`，Python 仍执行原位置/Euler 语义，只把复用的 index/position/Euler buffers 一次提交给 native。`ABI_VERSION` 保持 4；旧 DLL 依次回退 Transform batch 与 scalar setter。两套 Rust crate 各 13 个单元测试通过，PMX-Euler batch 与 scalar float bits 一致；当前 DLL SHA256：PMX `15D661F81824164BCE3628CD186B7E69AC5E0C6AB6EB77DB73E265E9715BCAFA`，MMD `82E272ECDBCAF64852B2C4B5246A1E85D404D57E152DD3473FDFBE7FE26E9123`，MMD IK bone solver `97FFA15BE62EAFCAA93C222FD646B46C9424B0BC959876E00778FB56F84B9DD9`。
- 生命周期与热路径同时改为精确 live binding/identity gate：普通 tick 不再周期性扫描全部 Rigid/Joint；Undo/Redo、rename/delete、Scene/Root/Armature identity 变化才重绑。Reset 先完成候选 solver、DisplayRig pose capture/apply 与 `view_layer.update()`，全部成功后才提交 world/session；失败会关闭候选 solver 并保留旧 solver/generation，不留下半提交状态。停止、保存、加载、异常清理均按 owner token 隔离，非 RNA 输入 fail closed。
- 最终 `04.blend` background 四象限连续平移 `全ての親` 60 tick、旋转 `足D.L` 60 tick 并静置 30 tick 全部通过：MMD/PMX × MMD IK 关/开均 `type0_error=0`，1/2 型动态与显示姿态每 tick 变化、最长冻结 0，Root/Foot authored error 不超过 `8.43e-08`，静置 drift 为 0；IK 开启时 Root native/显示响应均为 `0.06 m`，Foot native 旋转为 `0.35 rad`，排除 Foot 本身后的 descendant chain 响应为 `0.416830361`。type 0 oracle 从本 tick authored/native input 独立计算，不再与 DisplayRig 自指。
- 连续两轮最终四象限的 host frame median/p90（输入求值 + timer callback + 输出求值）存在机器调度波动：第一轮 MMD + IK 关/开为 `15.081/18.148 ms`、`17.535/19.963 ms`，PMX + IK 关/开为 `14.480/16.867 ms`、`16.782/19.244 ms`；提交前复跑分别为 `17.296/21.596 ms`、`21.128/23.658 ms`、`16.945/19.959 ms`、`20.269/24.644 ms`，中位约 `47.3-69.1 FPS`。更聚焦的 Pose Mode direct-input 热路径中，MMD + IK 为 `16.339/18.241 ms`，PMX + IK 为 `15.248/16.757 ms`。这些只是不含真实 viewport GPU draw/event loop 的 background host 指标，说明已进入 60 FPS 量级但尚未证明四组合稳定 60 FPS，不能冒充最终可见 GUI FPS 或新基线验收。
- 回归覆盖 DisplayRig/DebugBatch 生命周期、save/reload/orphan/identity/failure isolation、Reset 事务回滚、runtime transition、direct input、source VMD rewind、type 0 latency、MMD IK modal/feedback/reset/clear、PMX/MMD Root offset、07 Root motion 四组合、坐标变换和新 C ABI batch。Pose→Object 退出还覆盖 timeline-idle zero-step 强制最终矩阵刷新，以及刷新异常时解除 parking、恢复 presentation flags/step count 并保留原异常优先级。最终测试全程使用 `blender.exe --background`，未创建可见窗口、未抢前台焦点。
- 当前仅为后台验证通过的候选，不晋升新基线。版本保持 V0.1.8，不打包 ZIP、不 push、不移动 tag；真实 Blender 4.4 继续通过源码 Junction 使用。可见 Pose Mode 的四象限操作手感与 viewport FPS 仍需在用户明确允许无抢焦点 GUI 验收后单独确认。
