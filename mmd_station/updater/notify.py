...
MMD Station surfaces new releases as a calm in-panel banner at the top of the
``SPX_PT_SurfaceProxyCreator`` N-panel instead of the self-popping modal dialog. This module
...
def draw_update_banner(panel, context):
    """Draw the top-of-panel update banner; call first in SPX_PT_SurfaceProxyCreator.draw().
    ...
    row.operator(MMD_STATION_OT_UpdaterSkipSession.bl_idname, text="本次跳过")
    row.operator(MMD_STATION_OT_UpdaterDisableNotify.bl_idname, text="不再自动提醒")


class MMD_STATION_OT_UpdaterSkipSession(bpy.types.Operator):
    ...
    """
    bl_label = "本次跳过"
    bl_idname = "mmd_station.updater_skip_session"
...
class MMD_STATION_OT_UpdaterDisableNotify(bpy.types.Operator):
    """Turn off the host auto update-notification banner (auto_update_notify)."""
    bl_label = "不再自动提醒"
    bl_idname = "mmd_station.updater_disable_notify"
...
classes = (
    MMD_STATION_OT_UpdaterSkipSession,
    MMD_STATION_OT_UpdaterDisableNotify,
)
