...
# This host copy is vendored one level deeper, under mmd_station/updater/, so the
# engine's __init__ computes _addon_root as ".../mmd_station/updater" instead of
# ".../mmd_station". Left uncorrected, an update merges the new release into
# mmd_station/updater/ -- the real addon (mmd_station/__init__.py) is never
...
# true addon root: this file is mmd_station/updater/addon_updater_ops.py, so the
# addon root is its parent's parent.
if not getattr(updater, "invalid_updater", False):
    _mmd_station_addon_root = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))
    updater._addon_root = _mmd_station_addon_root
    updater._addon_package = "mmd_station"
    updater._updater_path = os.path.join(_mmd_station_addon_root, "mmd_station_updater")
...
def _draw_update_job_progress(layout):
    ...
    if hasattr(layout, "progress"):
        ...
        layout.label(text=iface("{}  总体 {}%".format(bar, percent)), icon="TIME")
        layout.label(text=iface(phase))
    layout.label(text=iface(detail))
...
def get_user_preferences(context=None):
    ...
    if not context:
        context = bpy.context
    # Host-level copy: look up the top-level addon ("mmd_station") rather than
...
class AddonUpdaterInstallPopup(bpy.types.Operator):
    ...
    def draw(self, context):
        ...
        if updater.invalid_updater:
            ...
            col = layout.column()
            col.scale_y = 0.7
            col.label(text=iface("更新 {} 已就绪！".format(updater.update_version)),
                      icon="LOOP_FORWARDS")
...
class AddonUpdaterCheckNow(bpy.types.Operator):
    ...
    bl_description = "立即检查 MMD Station 插件是否有更新"
    bl_options = {'REGISTER', 'INTERNAL'}
...
class AddonUpdaterUpdateNow(bpy.types.Operator):
    bl_label = "立即更新 MMD Station 插件"
    bl_idname = updater.addon + ".updater_update_now"
    bl_description = "更新到 MMD Station 插件的最新版本"
    bl_options = {'REGISTER', 'INTERNAL'}
    ...
    def execute(self, context):
        ...
        if updater.update_ready:
            if not _start_update_job(clean=self.clean_install):
                report(self, {'INFO'}, "更新正在进行中")
                return {'CANCELLED'}
            report(self, {'INFO'}, "更新已在后台开始，完成后会提示结果")
        elif updater.update_ready is None:
...
class AddonUpdaterUpdateTarget(bpy.types.Operator):
    bl_label = "MMD Station 指定版本"
    bl_idname = updater.addon + ".updater_update_target"
    bl_description = "安装 MMD Station 插件的指定版本"
    bl_options = {'REGISTER', 'INTERNAL'}
    ...
    def execute(self, context):
        ...
        if not _start_update_job(
            ...
            report(self, {'INFO'}, "更新正在进行中")
            return {'CANCELLED'}
        report(self, {'INFO'}, "指定版本已在后台开始安装，完成后会提示结果")
        return {'FINISHED'}
...
class AddonUpdaterUpdatedSuccessful(bpy.types.Operator):
    ...
    def draw(self, context):
        ...
        if self.error:
            ...
            col.label(text=iface(str(msg)), icon="BLANK1")
            rw = col.row()
            rw.scale_y = 2
            rw.operator(
                "wm.url_open",
...
class AddonUpdaterIgnore(bpy.types.Operator):
    ...
    def execute(self, context):
        ...
        if updater.invalid_updater:
            return {'CANCELLED'}
        updater.ignore_update()
        report(self, {"INFO"}, "在插件首选项中查看更新器选项")
        return {'FINISHED'}
...
def check_for_update_nonthreaded(self, context):
    ...
    if update_ready:
        ...
        report(self, {'INFO'}, "No update ready")
...
def update_settings_ui(self, context, element=None):
    ...
    if updater.invalid_updater:
        ...
        box.label(text=iface(updater.error_msg))
        return
    settings = get_user_preferences(context)
    if not settings:
    ...
    if updater.error is not None:
        ...
        split.operator(AddonUpdaterUpdateNow.bl_idname, text=iface(update_now_txt))
        split = sub_col.split(align=True)
        split.scale_y = 2
        split.operator(AddonUpdaterCheckNow.bl_idname,
                       text="", icon="FILE_REFRESH")
        ...
        split.operator(AddonUpdaterUpdateNow.bl_idname,
                       text=iface("立即更新到 " + str(updater.update_version)))
        split = sub_col.split(align=True)
        split.scale_y = 2
        split.operator(AddonUpdaterCheckNow.bl_idname,
                       text="", icon="FILE_REFRESH")
        ...
        col.operator("wm.url_open",
                     text=iface(dl_now_txt)).url = updater.website
    ...
    if not updater.manual_only:
        ...
        if updater.include_branches and len(updater.include_branch_list) > 0:
            ...
            col.operator(AddonUpdaterUpdateTarget.bl_idname,
                         text=iface("安装 {} / 旧版本".format(branch)))
        else:
            col.operator(AddonUpdaterUpdateTarget.bl_idname,
                         text="（重新）安装插件版本")
        last_date = "无"
        ...
        col.operator(AddonUpdaterRestoreBackup.bl_idname, text=iface(backup_text))
    ...
    if updater.error is not None and updater.error_msg is not None:
        row.label(text=iface(updater.error_msg))
    elif last_check:
        last_check = last_check[0: last_check.index(".")]
        row.label(text=iface("上次检查更新：" + last_check))
    else:
...
def update_settings_ui_condensed(self, context, element=None):
    ...
    if updater.error is not None:
        ...
        split.operator(AddonUpdaterUpdateNow.bl_idname, text=iface(now_txt))
        split = sub_col.split(align=True)
        split.scale_y = 2
        split.operator(AddonUpdaterCheckNow.bl_idname,
                       text="", icon="FILE_REFRESH")
        ...
        col.operator("wm.url_open", text=iface(dl_txt)).url = updater.website
    else:  # i.e. that updater.update_ready == False.
        sub_col = col.row(align=True)
        sub_col.scale_y = 1
        split = sub_col.split(align=True)
    ...
    if updater.error is not None and updater.error_msg is not None:
        ...
        last_check = last_check[0: last_check.index(".")]
        row.label(text=iface("Last check: " + last_check))
    else:
...
def select_link_function(self, tag):
    ...
    # MMD Station publishes its installable zip as a GitHub release ASSET
    # (built by pack.ps1; single top-level folder "mmd_station/"). Always prefer
    ...
    # the source zipball, whose folder layout differs from the asset.
...
def register():
    ...
    updater.user = "AliciaSource"
    updater.repo = "MMD-Station"
    # updater.addon = # define at top of module, MUST be done first
    updater.website = "https://github.com/AliciaSource/MMD-Station/releases"
    # MMD Station ships its install zip as a GitHub release asset (built by
    # pack.ps1); that asset's single top-level folder is "mmd_station/" and the
    # updater auto-descends into it, so no subfolder is needed.
    updater.subfolder_path = ""
    updater.current_version = bl_info["version"]
    updater.verbose = False  # make False for production default
    updater.backup_current = True  # True by default
    updater.backup_ignore_patterns = ["__pycache__", "mmd_station_updater"]
    updater.overwrite_patterns = ["*.dll", "*.png", "*.jpg", "*.md", "*.txt", "*.j2"]
    updater.remove_pre_update_patterns = ["*.py", "*.pyc"]
    # Only published GitHub Releases drive updates -- never branch tips. This
    # keeps ordinary dev pushes to main invisible to users.
