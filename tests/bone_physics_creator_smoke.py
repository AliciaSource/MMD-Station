...
root_bone_name = next(iter(armature.data.bones)).name
CHAIN_A = "Creator_A"
CHAIN_B = "Creator_B"
...
parent = armature.data.edit_bones[root_bone_name]
...
sibling.parent = armature.data.edit_bones[root_bone_name]
...
bpy.ops.object.mode_set(mode="POSE")
for bone_id, name in enumerate((root_bone_name, CHAIN_A, CHAIN_B, CHAIN_C, SIBLING)):
...
# Reordering moves only checked rows, not an implicit bone subtree.
actual_order = [bone.name for bone in sorted(armature.pose.bones,
                                            key=lambda bone: bone.mmd_bone.bone_id)]
assert actual_order == [root_bone_name, CHAIN_B, CHAIN_A, CHAIN_C, SIBLING], actual_order
...
for x, first, second in (
    ...
    first_bone.parent = armature.data.edit_bones[root_bone_name]
...
copy_source.mmd_rigid.bone = root_bone_name
...
copy_target = next(
    ...
    and rigid.mmd_rigid.bone == root_bone_name
