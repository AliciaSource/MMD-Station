...
const SNAPSHOT_BINDING_SIZE: usize = 32;
#[cfg(test)]
...
impl Solver {
    ...
    fn write_snapshot(&self) -> Result<Vec<u8>, String> {
        ...
        for binding in &self.bindings {
            ...
            push_u32(&mut output, binding.bone_rotation_overridden as u32);
        }
        Ok(output)
    }
    ...
    fn restore_snapshot(&mut self, input: &[u8]) -> Result<(), String> {
        ...
        self.world
            ...
            .map_err(|error| error.to_string())?;
        for (binding, (animation_bone, rotation_overridden)) in
...
fn read_transform(input: &[u8], cursor: &mut usize) -> Result<Transform, String> {
    let position = read_f32_array::<3>(input, cursor)?;
    let rotation = read_f32_array::<4>(input, cursor)?;
    Ok(Transform {
        position: Vec3 {
            x: position[0],
            y: position[1],
