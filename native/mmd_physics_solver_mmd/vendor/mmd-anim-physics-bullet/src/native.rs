...
impl BulletWorld {
    ...
    pub fn set_solver_iterations(&mut self, iterations: i32) -> Result<(), BulletError> {
        check(unsafe {
            ffi::mmd_anim_bullet_world_set_solver_iterations(self.raw.as_ptr(), iterations)
        })
    }
    ...
    pub fn rigidbody_matrix(
        &self,
        handle: RigidBodyHandle,
    ) -> Result<MatrixTransform, BulletError> {
        let mut position = [0.0; 3];
...
mod ffi {
    ...
    unsafe extern "C" {
        ...
        pub fn mmd_anim_bullet_world_set_solver_iterations(
            world: *mut World,
            iterations: i32,
        ) -> i32;
